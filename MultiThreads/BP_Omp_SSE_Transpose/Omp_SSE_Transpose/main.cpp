...
void backward(float* BatchData, float* Impl1, float* Impl2, float* yOut, float* yTag, float learningRate)//ÄæÌÝ¶È¸ÄÈ¨ 
{
	...
	float* Layer34T = (float*)malloc(ImplN * 10 * sizeof(float));
#pragma omp parallel for num_threads(THREADS_NUM)
	for (int i = 0; i < ImplN; i++)
		for (int j = 0; j < 10; j++)
			Layer34T[i * 10 + j] = Layer34[j * ImplN + i];
	//ÏÈËã3²ãDelta 
#pragma omp parallel for num_threads(THREADS_NUM)
	for (int i = 0; i < ImplN; i++)
	{
		...
		for (int j = 0; j < STEP * (10 / STEP); j += STEP)
		{
			...
			__m128 btmp = _mm_load_ps(Layer34T + i * 10 + j);
		...
		{
			//DeltaBef[i] += (DeltaAft[j] * Layer34[j * ImplN + i]);
			DeltaBef[i] += (DeltaAft[j] * Layer34T[i * 10 + j]);
		}
...
#pragma omp parallel for num_threads(THREADS_NUM)
	...
	float* Layer23T = (float*)malloc(ImplN * ImplN * sizeof(float));
#pragma omp parallel for num_threads(THREADS_NUM)
	for (int i = 0; i < ImplN; i++)
		for (int j = 0; j < ImplN; j++)
			Layer23T[i * ImplN + j] = Layer23[j * ImplN + i];
#pragma omp parallel for num_threads(THREADS_NUM)
	for (int i = 0; i < ImplN; i++)
	{
		__m128 resTmp = _mm_setzero_ps();
		...
		for (int j = 0; j < ImplN; j += STEP)
		{
			...
			__m128 btmp = _mm_load_ps(Layer23T + i * ImplN + j);
			btmp = _mm_mul_ps(atmp, btmp);
			resTmp = _mm_add_ps(resTmp, btmp);
		}
		resTmp = _mm_hadd_ps(resTmp, resTmp);
